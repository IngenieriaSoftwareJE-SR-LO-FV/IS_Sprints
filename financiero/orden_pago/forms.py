...
class OrdenPagoFinalForm(forms.ModelForm):
	class Meta:
		...
		widgets = {
			...
			"n_tramite":forms.TextInput(attrs={"class":"form-control"}),
			"fecha":forms.DateInput(attrs={"class":"form-control","type":"date", "value":date.today}),
			"estado":forms.Select(attrs={"readonly":True, "class":"select form-control"}),
			"tipo_proveedor":forms.Select(attrs={"readonly":True, "class":"select form-control"}),
			"proveedor":forms.Select(attrs={"readonly":True, "class":"select form-control"}),
			"centro_costos":forms.Select(attrs={"readonly":True, "class":"select form-control"}),
			"egreso":forms.Select(attrs={"readonly":True, "class":"select form-control"}),
			"tipo_comprobante":forms.Select(attrs={"readonly":True, "class":" select form-control"}),
			...
			"forma_pago":forms.Select(attrs={"readonly":True, "class":"select form-control"}),
			"observacion":forms.Textarea(attrs={"class":"form-control", "rows":2}),
